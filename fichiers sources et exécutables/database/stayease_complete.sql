...
CREATE DATABASE stayease CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE stayease;
...
CREATE TABLE IF NOT EXISTS logement (
    ...
    TITRE_LOG VARCHAR(200),
    DESCRIPTION_LOGEMENT TEXT,
    ADRESSE_LOG VARCHAR(255),
    VILLE_LOG VARCHAR(100),
    PRIX_LOG DECIMAL(10,2),
    NB_CHAMBRES INT DEFAULT 1,
    NB_SALLE_BAIN INT DEFAULT 1,
    CAPACITE_MAX INT DEFAULT 2,
    TYPE_LOG ENUM('hotel', 'maison', 'maison_hotes', 'ferme') DEFAULT 'hotel',
    EQUIPEMENTS_LOG TEXT,
    IMAGE_PRINCIPALE VARCHAR(500),
    STATUT_LOG ENUM('disponible', 'reserve', 'indisponible') DEFAULT 'disponible',
    NB_AVIS INT DEFAULT 0,
    NOTE_MOYENNE DECIMAL(3,2) DEFAULT 0,
    SAISON ENUM('haute saison', 'basse saison') DEFAULT 'basse saison',
...
CREATE TABLE IF NOT EXISTS logement_attraction (ID_LOGEMENT INT NOT NULL, ID_ATTRACTION VARCHAR(50) NOT NULL, PRIMARY KEY(ID_LOGEMENT,ID_ATTRACTION), FOREIGN KEY(ID_LOGEMENT) REFERENCES logement(ID_LOGEMENT) ON DELETE CASCADE, FOREIGN KEY(ID_ATTRACTION) REFERENCES attraction_et_reference(ID_ATTRACTION)) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS logement_evenement (ID_LOGEMENT INT NOT NULL, ID_EVENEMENT VARCHAR(50) NOT NULL, PRIMARY KEY(ID_LOGEMENT,ID_EVENEMENT), FOREIGN KEY(ID_LOGEMENT) REFERENCES logement(ID_LOGEMENT) ON DELETE CASCADE, FOREIGN KEY(ID_EVENEMENT) REFERENCES evenement(ID_EVENEMENT)) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS reservation (ID_RESERVATION INT PRIMARY KEY, ID_LOCATAIRE INT NOT NULL, ID_LOGEMENT INT NOT NULL, DATE_DEB DATE NOT NULL, DATE_FIN DATE NOT NULL, PRIX_TOTAL DECIMAL(10,2), STATUT ENUM('en_attente','acceptee','refusee','annulee') DEFAULT 'en_attente', FOREIGN KEY(ID_LOCATAIRE) REFERENCES locataire(ID_LOCATAIRE), FOREIGN KEY(ID_LOGEMENT) REFERENCES logement(ID_LOGEMENT)) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS avis_logement (ID_AVIS INT PRIMARY KEY, ID_LOCATAIRE INT NOT NULL, ID_RESERVATION INT NOT NULL, ID_LOGEMENT INT NOT NULL, NOTE_GLOBALE_LOG INT CHECK(NOTE_GLOBALE_LOG BETWEEN 1 AND 5), NOTE_PROPRETE_LOG INT CHECK(NOTE_PROPRETE_LOG BETWEEN 1 AND 5), NOTE_EMPLACEMENT_LOG INT CHECK(NOTE_EMPLACEMENT_LOG BETWEEN 1 AND 5), NOTE_RAPPORT_QUALITE_PRIX_LOG INT CHECK(NOTE_RAPPORT_QUALITE_PRIX_LOG BETWEEN 1 AND 5), CONTENU_LOG TEXT, TITRE_LOG VARCHAR(200), FOREIGN KEY(ID_LOCATAIRE) REFERENCES locataire(ID_LOCATAIRE), FOREIGN KEY(ID_RESERVATION) REFERENCES reservation(ID_RESERVATION), FOREIGN KEY(ID_LOGEMENT) REFERENCES logement(ID_LOGEMENT)) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS avis_attraction (ID_AVIS_ATT INT PRIMARY KEY, ID_LOCATAIRE INT NOT NULL, ID_ATTRACTION VARCHAR(50) NOT NULL, NOTE_GLOBALE_ATT INT CHECK(NOTE_GLOBALE_ATT BETWEEN 1 AND 5), CONTENU_ATT TEXT, DERNIERE_MAJ_ATT TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(ID_LOCATAIRE) REFERENCES locataire(ID_LOCATAIRE), FOREIGN KEY(ID_ATTRACTION) REFERENCES attraction_et_reference(ID_ATTRACTION)) ENGINE=InnoDB;
INSERT INTO avis_attraction VALUES 
...
CREATE TABLE IF NOT EXISTS avis_evenement (ID_AVIS INT PRIMARY KEY, ID_LOCATAIRE INT NOT NULL, ID_EVENEMENT VARCHAR(50) NOT NULL, NOTE_GLOBALE_EV INT CHECK(NOTE_GLOBALE_EV BETWEEN 1 AND 5), CONTENU_EV TEXT, FOREIGN KEY(ID_LOCATAIRE) REFERENCES locataire(ID_LOCATAIRE), FOREIGN KEY(ID_EVENEMENT) REFERENCES evenement(ID_EVENEMENT)) ENGINE=InnoDB;
